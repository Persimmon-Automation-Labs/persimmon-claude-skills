...
- **`DATABASE_URL="postgresql://noop..."`** — Prisma needs a parseable URL to generate the client. It does NOT connect during `prisma generate` or `prisma validate` or `next build` (as long as every DB-reading page has `export const dynamic = "force-dynamic"` — see `infra-railway-deploy` skill §9).
