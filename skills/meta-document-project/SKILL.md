...
Called by `meta-new-client-project`. Creates the full doc skeleton from templates below.
...
| `add adr {topic}` | Delegate to `meta-adr-authoring` skill |
...
### ADR — see `meta-adr-authoring` skill

Delegate ADR creation to the `meta-adr-authoring` skill. Do not inline ADR templates here.
