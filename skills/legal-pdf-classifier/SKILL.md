...
- `ai-sdk-wrapper` — provides `callClaude` with retries, caching, cost logs.
- `ai-prompt-library` — hosts `pageClassifierPrompt`, `PAGE_CLASSIFIER_TOOL`, `PAGE_CLASSIFIER_SYSTEM`.
- `legal-pt-prompting` — pt-BR register rules for the classifier's system prompt.
