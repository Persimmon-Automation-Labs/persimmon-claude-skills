...
Always `safeParse` through Zod. See the `stack-zod-boundary` skill.
...
- **stack-zod-boundary** — input schema patterns, `safeParse` discipline
- **data-prisma-pgvector** — the `db` client this skill assumes
- **security-nextauth** — the `auth()` call referenced above
- **stack-typescript-strict** — `ActionResult<T>` discriminated union, `satisfies`
