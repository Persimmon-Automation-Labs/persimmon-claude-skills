...
3. Railway injects these into the app service:
   ...
   - `BUCKET_NAME`
4. Configure CORS on the bucket via the `infra-s3-uploads` skill — Railway does NOT do this for you.
...
- **Fix**: See `infra-s3-uploads` skill. Add the origin and re-run `PutBucketCorsCommand`.
