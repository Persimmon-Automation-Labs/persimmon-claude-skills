...
- `ai-sdk-wrapper` — consumes every builder here via `callClaude`.
- `legal-pt-prompting` — pt-BR register rules for all Piccino prompts.
- `legal-brief-composer` — specific pipeline built on `briefComposerPrompt`.
- `legal-pdf-classifier` — specific pipeline built on `pageClassifierPrompt`.
