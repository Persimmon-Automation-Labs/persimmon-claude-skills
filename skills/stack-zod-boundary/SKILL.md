...
See `stack-typescript-strict` for the manual brand form.
...
- **stack-server-actions** — how the action boundary consumes these schemas
- **security-nextauth** — `authorize()` schema for login
- **data-prisma-pgvector** — validating embedding dim at the boundary
- **stack-typescript-strict** — branded types via `.brand<...>()`
