...
- `ai-prompt-library` — the `prompts.ts` counterpart. Every call to `callClaude` uses a prompt builder from there.
- `ai-rag-retrieval` — consumers of `callClaude` for composition pipelines.
- `legal-brief-composer` — uses `task: "compose-brief"` + prompt caching.
- `legal-pdf-classifier` — uses `task: "classify-page"` + tool-use.
